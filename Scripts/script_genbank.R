...
setwd("/home/yuri/Documentos/IC Evolução/Ecorregiões/América do Sul/Biomas/Amazônia")
list.files()

data1 <- read.table(file = "final_amazon.csv", header=T, sep = ",", dec = ".")
...
for (i in 1:linhas){
  ...
  write.fasta(sequences = xx, names = z, file.out = file.path(paste0(y,'.fasta')))
  }
